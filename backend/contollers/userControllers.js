...
const createUser = async (req, res) => {
  ...
  try {
    ...
    // Check if the department already has a dean or chairperson depending on the role being created
    if (role === "chairperson") {
        ...
        if (existingChairperson) {
          return res
            .status(400)
            .json({ message: "The department already has a chairperson" });
        }
      } else if (role === "dean") {
        const existingDean = await User.findOne({
          where: { role: "dean", departmentId },
        });
        if (existingDean) {
          return res.status(400).json({ message: "The department already has a dean" });
        }
      }
  ...
  try {
    ...
    if (userRole === "admin" && user.departmentId !== requesterDeptId) {
      return res
        .status(403)
        .json({
          message: "Admins can only delete users in their own department",
        });
    }
    ...
    if (userRole === "admin" || requesterId === parseInt(id, 10)) {
      ...
      if (userRole === "admin" && user.departmentId !== requesterDeptId) {
        return res
          .status(403)
          .json({
            message: "Admins can only view users in their own department",
          });
      }
...
const getAllUsers = async (req, res) => {
  ...
  try {
    ...
    if (currentUserRole === "admin" || currentUserRole === "manager") {
      ...
      if (!requesterDeptId) {
        console.error(
          "Department ID is undefined. Cannot filter users by department."
        );
        return res
          .status(400)
          .json({ message: "Department ID is required to fetch users" });
      }
      users = await User.findAll({
        where: {
          role: allowedRoles,
...
const promoteOrDemoteUser = async (req, res) => {
  ...
  try {
    ...
    if (newRole === "dean" && !["admin", "manager"].includes(userRole)) {
      return res
        .status(403)
        .json({ message: `Only Admins or Managers can promote to Dean` });
    }
    ...
    if (["admin", "manager"].includes(userRole)) {
      ...
      if (user.departmentId !== requesterDeptId) {
        return res
          ...
          .json({
            message:
              "You can only promote/demote users within your own department",
          });
      }
    }
    ...
    res
      .status(200)
      .json({
        message: `User promoted/demoted to ${newRole} successfully`,
        user,
      });
  } catch (error) {
